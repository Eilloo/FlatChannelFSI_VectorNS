...
ADCoupledPenaltyInterfaceDiffusionFieldSideOnly::validParams()
{
    ...
    params.addRequiredParam<Real>("component", "The x (0), y (1), or z (2) component represented by the field variable");
    params.addRequiredCoupledVar("primary_coupled_var", "The coupled field variable on the master side");
    params.addRequiredCoupledVar("secondary_coupled_vector_var", "The coupled vector variable on the slave side");
    return params;
}
...
ADCoupledPenaltyInterfaceDiffusionFieldSideOnly::ADCoupledPenaltyInterfaceDiffusionFieldSideOnly(
    ...
    _secondary_coupled_vector_value(adCoupledVectorNeighborValue("secondary_coupled_vector_var"))   //no non-ad version exists
...
ADCoupledPenaltyInterfaceDiffusionFieldSideOnly::computeQpResidual(Moose::DGResidualType type)
{
    ...
    {
        case Moose::Element:
            r = _test[_i][_qp] * _penalty * (_secondary_coupled_vector_value[_qp](_component).value() - _primary_coupled_value[_qp].value());
            break;
