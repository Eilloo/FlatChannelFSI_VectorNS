...
ADCoupledPenaltyInterfaceDiffusionVectorSideOnly::validParams()
{
  ...
  params.addRequiredCoupledVar("primary_coupled_vector_var",
                               "The coupled vector variable on the master side");
...
ADCoupledPenaltyInterfaceDiffusionVectorSideOnly::ADCoupledPenaltyInterfaceDiffusionVectorSideOnly(
  ...
  : ADVectorInterfaceKernel(parameters),
    ...
    _secondary_coupled_value_y(isCoupled("secondary_coupled_var_y")
                                   ? adCoupledNeighborValue("secondary_coupled_var_y")
                                   : _ad_zero),
    _secondary_coupled_value_z(isCoupled("secondary_coupled_var_z")
                                   ? adCoupledNeighborValue("secondary_coupled_var_z")
                                   : _ad_zero)
{
  // Constructor
}
...
ADCoupledPenaltyInterfaceDiffusionVectorSideOnly::computeQpResidual(Moose::DGResidualType type)
{
  ...
  {
    case Moose::Element:
      r_x = (_primary_coupled_vector_value[_qp](0) - _secondary_coupled_value_x[_qp]);
      if (_dim > 1)
      {
        r_y = (_primary_coupled_vector_value[_qp](1) - _secondary_coupled_value_y[_qp]);
      }
      if (_dim > 2)
      {
        r_z = (_primary_coupled_vector_value[_qp](2) - _secondary_coupled_value_z[_qp]);
      }
      r = _test[_i][_qp] * _penalty * ADRealVectorValue(r_x, r_y, r_z);
